document.addEventListener('DOMContentLoaded', () => {
    ...
    // This file is intentionally kept as the single owner of transaction direction UI.
    // Direction modes: buy, sell, mixed. Mixed uses two buttons per row.
    ...
    function renderRowDirection(tr) {
    ...
    const observer = new MutationObserver(() => {
        if (globalDirection === 'mixed') applyDirectionMode();
    });
    observer.observe(itemRows, {childList:true, subtree:true});

    // The existing transaction row creation/search/rate handlers remain in the page's inline scripts.
    // We only attach direction/payment behavior here.
    applyDirectionMode();
    ...
    function moneySafe(v) { return new Intl.NumberFormat('id-ID',{style:'currency',currency:'IDR',maximumFractionDigits:2}).format(Number(v)||0); }
    ...
    function calculateBalance() {
        ...
        const differenceBox=document.getElementById('summaryDifferenceBox'),settlementBox=document.getElementById('summarySettlementBox');
    ...
    function updateSplitLayout(){
        ...
        else if(method==='transfer'){transferField?.classList.add('show');bankField?.classList.add('show');}
        else{cashField?.classList.add('show');transferField?.classList.add('show');bankField?.classList.add('show');ensureSplitDecorations();}
    }
    function ensureSplitDecorations(){
        if(!paymentFields)return;
        if(!paymentFields.querySelector('.payment-split-note')){const note=document.createElement('div');note.className='payment-split-note';note.textContent='Split selalu menggunakan dua sumber: Cash + Transfer. Total keduanya harus sama dengan kebutuhan IDR.';paymentFields.prepend(note);}
        if(!paymentFields.querySelector('.payment-balance')){const balance=document.createElement('div');balance.className='payment-balance';balance.innerHTML='<span>Total dibayar</span><strong id="splitPaidTotal">Rp0</strong>';paymentFields.appendChild(balance);}
    }
    function updatePaymentRequirement(required){
        const requiredEl=document.getElementById('paymentRequired');if(requiredEl)requiredEl.textContent=moneySafe(required);
        const method=paymentMethodInput?.value||'cash';
        ...
        if(balance){const totalEl=balance.querySelector('#splitPaidTotal');if(totalEl)totalEl.textContent=moneySafe(total);const ok=method==='split'?Math.abs(total-required)<0.01:true;balance.classList.toggle('ok',ok);balance.classList.toggle('bad',!ok);}
        if(paymentError){if(method==='split'&&required>0&&Math.abs(total-required)>=0.01){paymentError.textContent=`Cash + Transfer harus sama dengan ${moneySafe(required)}.`;paymentError.style.display='block';}else paymentError.style.display='none';}
    }
    function setPaymentMethod(method){
        if(paymentMethodInput)paymentMethodInput.value=method;updateSplitLayout();
        ...
        else{const cash=Number(cashInput?.value)||0,transfer=Number(transferInput?.value)||0;if(cash===0&&transfer===0&&cashInput)cashInput.value=required.toFixed(2);}
    ...
    page.querySelector('#transactionForm')?.addEventListener('submit',event=>{
        const method=paymentMethodInput?.value||'cash',required=Math.abs(Number(document.getElementById('calculatedDifference')?.value)||0),cash=Number(cashInput?.value)||0,transfer=Number(transferInput?.value)||0;
