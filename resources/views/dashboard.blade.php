...
<div class="dashboard">
    ...
    <div class="activity-grid">
        ...
        <div class="activity-card"><div class="activity-icon">⇄</div><div class="activity-label">Mutasi Bank</div><div class="activity-value" data-dashboard="bank-balance">Rp 0</div><div class="activity-meta"><span class="neutral">Saldo Bank</span><span class="neutral" data-dashboard="bank-mutation-count">0 mutasi</span></div><div class="activity-meta"><span class="positive" data-dashboard="bank-credit-small">+ Rp 0</span><span class="negative" data-dashboard="bank-debit-small">- Rp 0</span></div><div class="activity-meta"><span class="neutral">Mutasi hari ini</span><span class="neutral" data-dashboard="bank-net-small">Rp 0</span></div></div>
    ...
    <div class="dashboard-card">
        <div class="dashboard-card-header"><div class="dashboard-card-title">Ringkasan Hari Ini</div><div class="dashboard-card-subtitle">Pergerakan saldo berdasarkan ledger</div></div>
        <div class="summary">
...
(function(){
    ...
    async function refreshDashboard(){
        try{
            ...
            set('purchase',data.today?.purchase);set('sales',data.today?.sales);set('cash-balance',data.position?.cash);set('bank-balance',data.position?.bank);set('bank-balance-position',data.position?.bank);set('forex-balance',data.position?.forex);set('gross',data.position?.gross);
            set('cash-in',data.today?.cash_in);set('cash-out',data.today?.cash_out);set('bank-credit',data.today?.bank_credit);set('bank-debit',data.today?.bank_debit);set('bank-credit-small',data.today?.bank_credit);set('bank-debit-small',data.today?.bank_debit);set('bank-net-small',data.today?.bank_net_change);set('expense',data.today?.expense);set('opening-cash',data.opening?.cash);set('opening-bank',data.opening?.bank);set('opening-forex',data.opening?.forex);
            setText('purchase-count',(data.today?.purchase_count||0)+' transaksi');setText('sales-count',(data.today?.sales_count||0)+' transaksi');setText('expense-count',(data.today?.expense_count||0)+' transaksi');setText('bank-mutation-count',(data.today?.bank_mutation_count||0)+' mutasi');
            const forexDetail=document.querySelector('[data-dashboard="forex-detail"]');if(forexDetail)forexDetail.textContent='Saldo awal '+money(data.opening?.forex)+' + beli '+money(data.today?.purchase)+' − jual '+money(data.today?.sales);
            const status=document.getElementById('dashboard-status');if(status){status.textContent='● Diperbarui '+new Date().toLocaleTimeString('id-ID');status.className='welcome-badge';}
        }catch(error){const status=document.getElementById('dashboard-status');if(status){status.textContent='● Gagal memuat saldo';status.className='welcome-badge';}console.error(error);}
    }
