...
<div class="container-fluid py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">Kelola rekening bank, posisi saldo, dan riwayat mutasi seluruh rekening.</p>
        </div>
    ...
    @else
        ...
        <div class="mutation-section card shadow-sm border-0">
            <div class="card-body border-bottom">
                <div class="mutation-title">Riwayat Mutasi Semua Bank</div>
                <div class="mutation-subtitle">Seluruh mutasi rekening pada cabang aktif, terbaru di atas.</div>
            </div>

            @if($mutations->isEmpty())
                <div class="mutation-empty">Belum ada riwayat mutasi bank.</div>
            @else
                    ...
                    <table class="table table-hover mutation-table mb-0">
                        ...
                        @foreach($mutations as $mutation)
                            <tr>
                                ...
                                <td>
                                    <div class="mutation-bank">{{ $mutation->bankAccount?->bank_name ?? 'Bank' }}</div>
                                    <div class="mutation-account">{{ $mutation->bankAccount?->account_number ?? '-' }}</div>
                                </td>
                                ...
                                <td>
                                    @if($mutation->reconciliation_status === 'matched')
                                        <span class="badge bg-success">Matched</span>
                                    @elseif($mutation->reconciliation_status === 'manual')
                                        <span class="badge bg-info text-dark">Manual</span>
                                    @elseif($mutation->reconciliation_status === 'unmatched')
                                        <span class="badge bg-warning text-dark">Unmatched</span>
                                    @else
                                        <span class="badge bg-secondary">{{ $mutation->reconciliation_status ?: '-' }}</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
